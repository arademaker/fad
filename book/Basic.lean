...
def savedLean : CodeBlockExpander
  | args, code => do
    ...
    return #[← ``(Block.other
      (Block.savedLean $(quote (← getFileName)) $(quote (code.getString))) #[$underlying,*])]
...
def savedImport : CodeBlockExpander
  | args, code => do
    ...
    return #[← ``(Block.other
      (Block.savedImport $(quote (← getFileName)) $(quote (code.getString))) #[])]
...
def savedComment : CodeBlockExpander
  | args, code => do
    ...
    let comment := s!"/-!\n{str}\n-/"
    return #[← ``(Block.other
      (Block.savedLean $(quote (← getFileName)) $(quote comment)) #[])]
